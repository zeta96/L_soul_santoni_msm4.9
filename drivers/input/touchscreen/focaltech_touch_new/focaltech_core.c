...
#if defined(CONFIG_FTS_TRUSTED_TOUCH_NEW)
...
#ifdef CONFIG_FTS_TRUSTED_TOUCH_NEW
...
static void FTS_TRUSTED_TOUCH_NEW_intr_gpio_toggle(struct fts_ts_data *fts_data,
		bool enable)
{
	void __iomem *base;
	u32 val;
...
static void fts_ts_trusted_touch_tvm_vm_mode_enable(struct fts_ts_data *fts_data)
{
	...
	FTS_TRUSTED_TOUCH_NEW_intr_gpio_toggle(fts_data, false);
	if (irq < 0) {
...
static void FTS_TRUSTED_TOUCH_NEW_pvm_vm_mode_disable(struct fts_ts_data *fts_data)
{
	int rc = 0;
...
int fts_ts_handle_trusted_touch_pvm(struct fts_ts_data *fts_data, int value)
{
	...
	case 0:
		...
		if (atomic_read(&fts_data->trusted_touch_mode) ==
				TRUSTED_TOUCH_VM_MODE) {
			FTS_TRUSTED_TOUCH_NEW_pvm_vm_mode_disable(fts_data);
		} else {
			pr_err("Unsupported trusted touch mode\n");
		}
		break;
